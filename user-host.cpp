...
int main(int argc, char** argv) {
    ...
    //auto ip1_ddr = xrt::bo(device, vector_size_bytes, 1);

    // Map the contents of the buffer object into host memory
    auto bo0_map = ip1_boA.map<int*>();
    auto bo1_map = ip1_boB.map<int*>();
    //auto ddr_map = ip1_ddr.map<int*>();
 
    std::fill(bo0_map, bo0_map + DATA_SIZE, 0);
    std::fill(bo1_map, bo1_map + DATA_SIZE, 0);
    //std::fill(ddr_map, ddr_map + DATA_SIZE, 0);

    // Create the test data
    int bufReference[DATA_SIZE];
    //int bufReferenceDDR[DATA_SIZE];
    ...
    //std::cout << "Setting Register \"DDR\" (Input Address)" << std::endl;
    //ip1.write_register(DDR_OFFSET, buf_addr[2]);
    //ip1.write_register(DDR_OFFSET + 4, buf_addr[2] >> 32);
    ...
    //ip1_ddr.sync(XCL_BO_SYNC_BO_FROM_DEVICE);

    // Validate results
    if (std::memcmp(bo1_map, bufReference, DATA_SIZE))
        throw std::runtime_error("Value read back does not match reference");
    std::cout << "Vadd PASSED\n";
/*
    int c = 0;
    for(c = 0; c < 48; c++){
		std::cout << bo1_map[c] << std::endl;
	}

	for(c = 0; c < 48; c++){
		std::cout << ddr_map[c] << std::endl;
	}
    if (std::memcmp(ddr_map, bufReferenceDDR, DATA_SIZE))
        throw std::runtime_error("DDR Value read back does not match reference");
        */
